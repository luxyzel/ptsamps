...
class CreateAssetsArchivedTrigger extends Migration
{
    ...
    public function up()
    {
        ...
        BEGIN
         INSERT INTO assets_archived (`category_type`, `model`, `st_msn`, `pdsn`, `asset_tag`, `asset_number`, `adapter`, `location`, `ws_no`, `st`, `s_n`, `mouse`, `keyboard`, `code`, `description`, `condition`, `status`, `date_delivered`, `warranty_ends`, `vendor`, `notes`, `created_at`, `updated_at` ) VALUES (old.category_type, old.model, old.st_msn, old.pdsn, old.asset_tag, old.asset_number, old.adapter, old.location, old.ws_no, old.st, old.s_n, old.mouse, old.keyboard, old.code, old.description, old.condition, old.status, old.date_delivered, old.warranty_ends, old.vendor, old.notes, now(), now());
        END
        ');
    }
